...
Agents are pluggable too, behind an `AgentRunner` trait, configured via the `project.agent` key. Besides `claude` (the default) and `noop` (does nothing, for dry runs), there's `lmstudio`, which drives a local [LM Studio](https://lmstudio.ai/) server instead: it talks to LM Studio's OpenAI-compatible `/chat/completions` endpoint and gives the model tool-calling access to read/write files and run shell commands in the project checkout, looping until the model replies without any further tool calls. See `config.yaml.example` for its settings (`base_url`, `model`, `max_iterations`, `timeout_secs`).
